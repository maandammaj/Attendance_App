...
class ReportCsvBuilder {
  ...
  static String attendance(AnalyticsReport report) {
    ...
    for (final entry in byDate.entries) {
      ...
      rows.add([
        ...
        '${cell.sessionCount}',
        cell.firstCheckIn == null ? '' : DateHelpers.formatTime(cell.firstCheckIn!),
        cell.lastCheckOut == null ? '' : DateHelpers.formatTime(cell.lastCheckOut!),
  ...
  static String salary(AnalyticsReport report) {
    ...
    return _encode([
      ...
      ['الساعات المطلوبة', (report.attendance.totalRequiredMinutes / 60).toStringAsFixed(2)],
      ['الساعات المنجزة', (report.attendance.totalWorkedMinutes / 60).toStringAsFixed(2)],
      ['إجمالي التواجد', (report.attendance.totalPresenceMinutes / 60).toStringAsFixed(2)],
      ['ساعات إضافية', (report.attendance.totalOvertimeMinutes / 60).toStringAsFixed(2)],
      ['ساعات عجز وغياب', (report.attendance.totalDeficitMinutes / 60).toStringAsFixed(2)],
      ['أيام الحضور', '${report.attendance.attendedDays}'],
      ['أيام الغياب', '${report.attendance.absentDays}'],
