...
description: DO NOT USE IN THIS PROJECT — mockito is not a dependency; use hand-written fakes (rule 14). Define and generate mock objects for external dependencies using `package:mockito` and `build_runner`. Use when unit testing classes that depend on complex external services like APIs or databases.
metadata:
  model: models/gemini-3.1-pro-preview
  last_modified: Fri, 24 Apr 2026 15:13:58 GMT
---

> ## ⚠️ Project override — DO NOT APPLY
>
> **`mockito` is not a dependency of this project and adding it needs explicit approval.** The
> established pattern is a hand-written fake: a small class implementing the interface with
> test-controlled state — see any `test/features/<feature>/<feature>_test_doubles.dart`.
>
> Reach for a mocking framework only when a hand-written fake has become genuinely unwieldy, and add
> the dependency deliberately. Governing rule: `.claude/rules/14-testing.md`.
