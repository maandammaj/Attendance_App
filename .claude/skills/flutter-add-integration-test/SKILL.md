...
description: NEEDS APPROVAL IN THIS PROJECT — integration_test is not declared and not in CI (rule 14). Configures Flutter Driver for app interaction and converts MCP actions into permanent integration tests. Use when adding integration testing to a project, exploring UI components via MCP, or automating user flows with the integration_test package.
metadata:
  model: models/gemini-3.1-pro-preview
  last_modified: Tue, 21 Apr 2026 18:29:20 GMT
---

> ## ⚠️ Project override — NEEDS APPROVAL
>
> **`integration_test` is not part of this project's pipeline and is not declared in
> `pubspec.yaml`.** CI runs `flutter analyze` and `flutter test` only.
>
> Adding an integration test means adding the dependency and a CI stage — confirm with the user
> first. Governing rule: `.claude/rules/14-testing.md`.
