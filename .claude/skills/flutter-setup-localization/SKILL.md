...
description: DO NOT USE IN THIS PROJECT — localization is LangConfig + lib/config/locales/{ar,en}.json + <feature>_strings.dart, not intl/ARB (rule 17). Add `flutter_localizations` and `intl` dependencies, enable "generate true" in `pubspec.yaml`, and create an `l10n.yaml` configuration file. Use when initializing localization support for a new Flutter project.
metadata:
...
>
> **This project does not use `flutter_localizations` + `intl` + `l10n.yaml` ARB generation.** Copy
> lives in `lib/config/locales/ar.json` and `en.json` (a key missing from either file is a bug), is
> resolved context-free through `LangConfig.instance.data[key]`, and every feature reaches it through
> one typed accessor: `lib/features/<feature>/presentation/<feature>_strings.dart`.
>
> Governing rule: `.claude/rules/17-accessibility.md`.
