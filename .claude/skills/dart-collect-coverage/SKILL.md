...
description: ADAPT FOR THIS PROJECT — use `fvm flutter test --coverage`; do not add the coverage package (rule 14). Collect coverage using the coverage packge and create an LCOV report
metadata:
...
>
> **Do not add the `coverage` package.** Flutter collects coverage natively:
>
> ```bash
> fvm flutter test --coverage      # writes coverage/lcov.info
> ```
>
> The LCOV report generation below still applies; the `package:test` / `coverage` setup steps do not.
