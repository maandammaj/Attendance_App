...
description: DO NOT USE IN THIS PROJECT — package:checks is not declared; assertions use flutter_test's expect (rule 14). Replace the usage of `expect` and similar functions from `package:matcher` to `package:checks` equivalents.
metadata:
...
>
> **`package:checks` is not declared in this project and there is nothing to migrate to it.**
> Assertions use `expect(...)` from `flutter_test`, which all 194 existing tests are written against.
>
> Governing rule: `.claude/rules/14-testing.md`.
