...
description: SUPERSEDED IN THIS PROJECT — serialization is json_serializable + build_runner, models in lib/core/items/ (rules 11, 13). Create model classes with `fromJson` and `toJson` methods using `dart:convert`. Use when manually mapping JSON keys to class properties for simple data structures.
metadata:
  model: models/gemini-3.1-pro-preview
  last_modified: Tue, 21 Apr 2026 21:44:50 GMT
---

> ## ⚠️ Project override — SUPERSEDED
>
> **This project does not hand-write `fromJson`/`toJson` with `dart:convert`.** Serialization is
> `json_serializable` + `json_annotation` with generated `*.g.dart` siblings, regenerated via
> `fvm dart run build_runner build --delete-conflicting-outputs`. Models annotate
> `@JsonSerializable(fieldRename: FieldRename.snake)` and live in `lib/core/items/`.
>
> Governing rules: `.claude/rules/11-data-handling.md` and `13-code-generation.md`. Before writing
> any model, check whether `lib/core/items/` already has a DAO for the shape
> (`.claude/rules/20-naming-and-file-organization.md`).
