...
description: ADAPT FOR THIS PROJECT — use flutter_test (not package:test) and hand-written fakes (rule 14). Write and organize unit tests for functions, methods, and classes using `package:test`. Use when creating new logic or fixing bugs to ensure code remains correct and regression-free.
metadata:
...
>
> **Framework:** this project tests with `flutter_test` (bundled with the SDK) — **not**
> `package:test`. `mockito`, `mocktail`, `package:checks` and `integration_test` are not declared and
> must not be imported without explicit approval.
>
> **Mocking:** hand-written fakes — a small class implementing the interface with test-controlled
> state. See `test/features/<feature>/<feature>_test_doubles.dart` for the established shape.
>
> **Location:** `test/` mirrors `lib/`. Governing rule: `.claude/rules/14-testing.md`.
