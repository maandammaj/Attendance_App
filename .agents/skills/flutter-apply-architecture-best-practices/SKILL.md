...
description: SUPERSEDED IN THIS PROJECT — architecture is defined in .claude/rules/08-application-architecture.md; use the feature-to-bloc-clean-arch skill. Architects a Flutter application using the recommended layered approach (UI, Logic, Data). Use when structuring a new project or refactoring for scalability.
metadata:
...
>
> **This project's architecture is already decided and documented.** Read
> `.claude/rules/08-application-architecture.md` for what it is, ADR-003 for why, and the
> `feature-to-bloc-clean-arch` skill for how to move a feature onto it.
>
> Two concrete divergences from the generic guidance below: `freezed` is **not** adopted (Dart
> `sealed` classes + `equatable` cover states and failures), and dependency injection is `get_it`
> wired **by hand** — no `injectable`, no codegen.
