...
description: DO NOT USE IN THIS PROJECT — go_router/auto_route are rejected; routing is named routes in lib/core/utils/routes.dart (rule 10). Configure `MaterialApp.router` using a package like `go_router` for advanced URL-based navigation. Use when developing web applications or mobile apps that require specific deep linking and browser history support.
metadata:
...
>
> **This project does not use `go_router` or `auto_route`, and will not adopt them.** Navigation is
> the named-route `Navigator` with routes registered in `lib/core/utils/routes.dart`; deep linking
> goes through `app_links` + `routes.dart`.
>
> Governing rule: `.claude/rules/10-state-management-and-routing.md`. The rejection is recorded in
> the settled-decisions table of `docs/architecture/target-architecture.md` — do not re-propose it.
>
> Read below only as background on declarative routing in general.
